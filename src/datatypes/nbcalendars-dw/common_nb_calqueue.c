...
__thread unsigned long long pro_cache_usage = 0;
...
void pq_report(int TID)
{
	...
	printf("TID %d: elementi bloccati %d, cache hit %ld, tentativi estrazione bucket remoti %ld, estrazioni remote svolte %ld, ricerche risparmiate in pro %llu \n", TID, blocked, cache_hit, remote_node_dequeue, remote_node_dequeue_exec, pro_cache_usage);
	#if NUMA_DW || SEL_DW
