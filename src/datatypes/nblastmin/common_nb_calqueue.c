...
unsigned int search_and_insert(nbc_bucket_node *head, double timestamp, unsigned int tie_breaker,
						 int flag, nbc_bucket_node *new_node_pointer, nbc_bucket_node **new_node, bool do_search)
{
...
void migrate_node(nbc_bucket_node *right_node,	table *new_h)
{
	...
	search_and_insert(bucket, new_node_timestamp, new_node_counter, REMOVE_DEL, new_node_pointer, new_node, true)
	) == ABORT);
...
void pq_enqueue(nb_calqueue* queue, double timestamp, void* payload)
{
	...
	while(res != OK){
		...
		if(res == MOV_FOUND){
			...
			do_search = (current - newIndex ) < 0;
			do_search = do_search || ( (current - newIndex ) >= 2*MAX_SKIP_BUCKETS );

		}
		// search the two adjacent nodes that surround the new key and try to insert with a CAS 
	    res = search_and_insert(bucket, timestamp, 0, REMOVE_DEL_INV, new_node, &new_node,  do_search);
	}
