...
int search_and_insert(nbc_bucket_node *head, double timestamp, unsigned int tie_breaker,
						 int flag, nbc_bucket_node *new_node_pointer, nbc_bucket_node **new_node, double bucket_width)
{
	nbc_bucket_node *left, *left_next, *tmp, *tmp_next, *tail, *new_head;
	unsigned int counter;
	...
	do
	{
		...
		// Check for missing head of the virtual bucket
		if(left->node_type != HEAD && hash(left_timestamp, bucket_width) < hash(timestamp, bucket_width)){
			new_head = node_malloc(NULL, hash(timestamp, bucket_width)*bucket_width,0);
			new_head->node_type = HEAD;
			new_head->next = new_node_pointer;
			if (BOOL_CAS(&(left->next), left_next, get_marked(new_head,get_mark(left_next))))
			{
				if(is_new_key)
				{
		...
		// copy left node mark			
		else if (BOOL_CAS(&(left->next), left_next, get_marked(new_node_pointer,get_mark(left_next))))
		{
			if(is_new_key)
			{
				scan_list_length_en += len;
	...
	{   
		...
		for (i = 0; i < size; i++)
		{
			...
			while( tmp != tail && counter < sample_size )
			{
				...
				if(!is_marked(tmp_next, DEL) && !is_marked(tmp_next, INV) && tmp->node_type == ITEM)
				{
...
void migrate_node(nbc_bucket_node *right_node,	table *new_h)
{
	...
	search_and_insert(bucket, new_node_timestamp, new_node_counter, REMOVE_DEL, new_node_pointer, new_node, new_h->bucket_width)
	) == ABORT);
...
void* pq_init(unsigned int threshold, double perc_used_bucket, unsigned int elem_per_bucket)
{
	...
	for (i = 0; i < MINIMUM_SIZE; i++)
	{
		...
		res->hashtable->array[i].node_type = HEAD;
	}
...
int pq_enqueue(void* q, pkey_t timestamp, void* payload)
{
	...
	while(res != OK){
	    ...
	    res = search_and_insert(bucket, timestamp, 0, REMOVE_DEL_INV, new_node, &new_node, h->bucket_width);
	}
