...
volatile int _fc_lock = 0;
char padd[CACHE_LINE_SIZE];
 ...
 */
int do_enqueue(void* q, pkey_t timestamp, void* payload)
{
 ...
 *
 */
pkey_t do_dequeue(void *q, void** result)
{
	nb_calqueue *queue = (nb_calqueue*)q;
...
void flat_combining(void* q) 
{
	
	/* Preamble */

	slotInfo_t* curr_slot;
	int type, attempts, num_changes;
	pkey_t ret_ts;
	void* ret_ps;
	for (attempts = 0; attempts < 2; ++attempts) 
	{
		num_changes = 0;
		curr_slot = _tail_slot;

		IRMB();

		while (curr_slot->next != NULL && curr_slot->next != curr_slot)
		{
			type = curr_slot->request.type;
			if (type == 1) // ENQ
			{
				num_changes++;
				do_enqueue(q, curr_slot->request.timestamp, curr_slot->request.payload);
				curr_slot->request.ret_value = 1;
				curr_slot->request.type = -1;
			}
			else if (type == 0) // DEQ
			{
				num_changes++;
				ret_ts = do_dequeue(q, &ret_ps);
				curr_slot->request.ret_value = 1;
				curr_slot->request.timestamp = ret_ts;
				curr_slot->request.payload = ret_ps;
				// memory barrier
				curr_slot->request.type = -1;
			}
			else // INV
...
int pq_enqueue(void* q, pkey_t timestamp, void* payload) 
{
	critical_enter();
	slotInfo_t* my_next;
	slotInfo_t* my_slot = _tls_slot_info;
	if (my_slot == NULL)
		my_slot = get_new_slot();

	my_next = my_slot->next;
	my_slot->request.type = 1; // ENQ
	my_slot->request.timestamp = timestamp;
	my_slot->request.payload = payload;
	my_slot->request.ret_value = -1;
	do 
	{
		if (my_next == NULL)
			enq_slot(my_slot);
		
		if (BOOL_CAS(&_fc_lock, 0 ,1))
		{
			// lock acquired
			flat_combining(q);
			_fc_lock = 0;
		...
		else
		{
			// cannot acquire lock
			IWMB();
			while(my_slot->request.ret_value == -1 && _fc_lock != 0)
				sched_yield();
			
			IRMB();
			if (my_slot->request.ret_value != -1)
			{
				critical_exit();
				return 1;
			}
...
pkey_t pq_dequeue(void *q, void** result) 
{
	...
	do 
	{
		...
		if (BOOL_CAS(&_fc_lock, 0 ,1))
		{
			...
			flat_combining(q);
			_fc_lock = 0;
			*result = my_slot->request.payload;
			critical_exit();
			return my_slot->request.timestamp;
		}
		else
		{
			// cannot acquire lock
