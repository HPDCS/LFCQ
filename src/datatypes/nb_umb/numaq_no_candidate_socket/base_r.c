...
int do_pq_enqueue(void* q, pkey_t timestamp, void* payload, int override, unsigned int* new_dest)
{
	...
	unsigned long dest_node, dest_socket;
...
int pq_enqueue(void* q, pkey_t timestamp, void *payload) 
{
	...
	unsigned int dest_node, new_dest, dest_socket;	 
	...
	do {
		...
		{	
			...
			if (!tq_dequeue(&op_queue[SID], &operation)) 
			{
				operation = requested_op;
				mine = true;
			}
		...
		{
			ret = do_pq_enqueue(q, operation->timestamp, operation->payload, mine, &new_dest);
		...
		{
			ret = do_pq_dequeue(q, &ret_ts, &new_payload, mine, &new_dest);
			if (ret != -1)
			{
				performed_dequeue++;
