...
int __attribute__((optimize("O0"))) single_step_pq_enqueue(table *h, pkey_t timestamp, void *payload)
{
...
int __attribute__((optimize("O0"))) single_step_pq_dequeue(table *h, nb_calqueue *queue, pkey_t* ret_ts, void **result)
{
    unsigned long i = LOOP_COUNT;
    while(i > 0)
		i--;
