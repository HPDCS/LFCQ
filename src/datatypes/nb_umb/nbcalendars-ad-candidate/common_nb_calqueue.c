...
void migrate_node(nbc_bucket_node *right_node,	table *new_h)
{
	...
	replica = numa_node_malloc(right_node->payload, right_node->timestamp,  right_node->counter, NODE_HASH(index % new_h->size));
...
void* pq_init(unsigned int threshold, double perc_used_bucket, unsigned int elem_per_bucket)
{
	...
	gc_aid[1] = gc_add_allocator(sizeof(op_node));
	...
	res->tail = numa_node_malloc(NULL, INFTY, 0, NID);
	res->tail->next = NULL;
...
int pq_enqueue(void* q, pkey_t timestamp, void* payload)
{
	...
	critical_enter();
	op_node* operation = gc_alloc_node(ptst, gc_aid[1], NID);
	operation->op_id = 1;
	...
	operation->requestor = operation;

	
	int ret = do_pq_enqueue(q, operation->timestamp, operation->payload, &operation->candidate, operation);
	gc_free(ptst, operation, gc_aid[1]);
	critical_exit();
