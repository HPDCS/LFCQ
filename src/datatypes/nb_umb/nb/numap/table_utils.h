...
static inline void migrate_node(nbc_bucket_node *right_node,	table *new_h)
{
	...
	if (right_node->op_id == 1) {
		operation_t * op = right_node->requestor;
		if (!BOOL_CAS(&(op->candidate), NULL, right_node))
		{
			...
			do {
				...
				new_state.op_id = 0;
			} while(VAL_CAS(&right_node->widenext, curr_state.widenext, new_state.widenext) != curr_state.widenext);
