...
int do_pq_enqueue(void *q, pkey_t timestamp, void *payload, nbc_bucket_node* volatile * candidate, operation_t *operation, int override, unsigned int* new_dest)
{
	...
	for (j = 0; j < ACTIVE_NUMA_NODES; j++)
	{
		...
		if (read_slot(to_me, &operation))
		{
			...
			if (type == OP_PQ_ENQ) 
			{
				...
				ret = do_pq_enqueue(q, ts, pld, candidate, operation, 0, &new_dest);
			...
			else 
			{
				...
				ret = do_pq_dequeue(q, &ts, &pld, op_id, candidate, 0, &new_dest);
			...
			if (ret < 0)
			{
				...
				ret = -2; // locality has changed - comunicate it
			}

			if (!BOOL_CAS(&operation->response, -1, ret))
				continue; // the cas failed -> someone already set the response
...
int pq_enqueue(void* q, pkey_t timestamp, void* payload) {
	...
	my_operation->dest_node = dest_node;
...
pkey_t pq_dequeue(void *q, void** result)
{
	...
	if (dest_node == NID) {
		ret = do_pq_dequeue(q, &ts, result, my_operation->op_id, &my_operation->candidate, 1, NULL);
		gc_free(ptst, my_operation, gc_aid[1]);
		critical_exit();
		return ts;
	}
