    ...
    op_payload *operation) 
{
    ...
    if (val == 0)
    {
        operation->type         = slot_arr[current].type;
        operation->ret_value    = slot_arr[current].ret_value;
        operation->timestamp    = slot_arr[current].timestamp; 
        operation->payload      = slot_arr[current].payload;
        operation->candidate    = slot_arr[current].candidate;
        operation->op_id        = slot_arr[current].op_id;
        operation->requestor    = slot_arr[current].requestor;
    ...
    op_payload *operation)
{
    ...
    // write on slot
    slot_arr[index].type        = operation->type;
    slot_arr[index].ret_value   = operation->ret_value;
    slot_arr[index].timestamp   = operation->timestamp; 
    slot_arr[index].payload     = operation->payload;
    slot_arr[index].candidate   = operation->candidate;
    slot_arr[index].op_id       = operation->op_id;
    slot_arr[index].requestor   = operation->requestor;
