 ...
 */
int do_pq_enqueue(void* q, pkey_t timestamp, void* payload, op_payload * operation)
{
	...
	nbc_bucket_node *bucket, *ins_node, *new_node = numa_node_malloc(payload, timestamp, 0, NID);
	table * h = NULL;		
	...
	{
		// CAS failed
		// someone else set the candidate
		// mark my node as DEL 
		do {
			...
			new_state.next = get_marked(new_node->next, DEL);
			new_state.op_id = 0;
	...
	// change this cas to a cas128 (maybe this causes some problem)
	if (!BOOL_CAS(&(ins_node->op_id), 1, 0))
			return 1;
 ...
 *
 */
pkey_t do_pq_dequeue(void *q, void** result, op_payload * operation)
{
...
begin:
	...
	{	
		...
		do
		{
			...
			if (left_node_op_id == 1) continue; // in the mean time the node could has been validated
			...
			{
				// se sono qui, qualcuno ha già estratto un minimo (o sta provando ad estrarlo) per la mia operazione
				// e left è stato inserito prima del candidato.
				if (current_candidate == (nbc_bucket_node*) 0x1ull)
				{
					// one of my companion said it was empty
					*result = NULL;
					return INFTY;
				}

				// else try to extract the candidate
				lnn.next = current_candidate->next;
				lnn.op_id = 0;

				new.next = get_marked(lnn.next, DEL);
				new.op_id = op_id;
				...
				if (is_marked(current_candidate->next, DEL))
				{
					...
					else
					{
						...
						*result = NULL;
						continue; //goto begin; // con il continue provo ad estrarre il nuovo minimo, è inutile ricominciare da zero
					}
				}
			...
			// the node cannot be extracted && is marked as DEL => check who extracted it!
	...
	for (i = NID+1; i % ACTIVE_NUMA_NODES != NID; i++)
	{
		...
		if (read_slot(to_me, &operation))
		{
			...
			if (type == OP_PQ_ENQ) 
			{
				...
				ret = do_pq_enqueue(q, ts, pld, &operation);
				operation.ret_value = ret;
			}
			else 
			{
				ts = do_pq_dequeue(q, &pld, &operation);
				operation.timestamp = ts;
				operation.payload = pld;
				operation.ret_value = 1; // no needed
			}
