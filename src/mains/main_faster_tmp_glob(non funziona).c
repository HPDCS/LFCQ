...
int NUMA_NODES;
volatile pkey_t global_timestamp = 0.0;
		...
		)
{
	...
	while(iter < iterations)
	{
		...
		pkey_t aus_tmp, local_timestamp = 0.0;
		int count = 0;
		int max_count = 30;

        if(my_id % 2 == 0)        	
        	BOOL_CAS_DOUBLE(&global_timestamp, 0.0, local_min);
        //else
        //	while(global_timestamp == 0.0);

        //printf("TID %d %f\n", TID, global_timestamp);
		...
		while((TEST_MODE != 'T' && tot_count < end_operations2) || (TEST_MODE == 'T' && !end_test))
		{
			if(my_id % 2 == 0){
				...
				if(count == max_count){
					count = 0;
					if(local_timestamp > (aus_tmp = global_timestamp))
							BOOL_CAS_DOUBLE(&global_timestamp, aus_tmp, local_timestamp);
				}
				//pthread_yield();
			}
			...
			if(my_id % 2 == 1){
				par_count++;	
				if(global_timestamp == 0.0)		aus_tmp = local_min;
				else 							aus_tmp = global_timestamp;
                enqueue(my_id, seed, aus_tmp, current_dist);
			}
		...
		for(j = 0; j < THREADS; j++)
			printf("slfjsldj TID %d %lld %lld\n\n\n\n\n",TID, par_count, tot_count );
