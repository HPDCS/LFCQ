...
for u in $usage_factor; do
	...
	out3=$output/"$version-$cmd-1-$DIST-0.3-$SIZE-$DIST-0.5-$OPS-$DIST-0-0-$u-0-$MODE-$TIME-START.dat"
	line="THREADS"
		...
		for p in $data_types; do
			...
			for e in `eval echo '$'elem_per_bucket_$p`; do
				...
				for i in $iterations; do
					...
					val=`grep THROUG $file  | cut -d',' -f4 | cut -d':' -f2`
					if [ "$val" == "" ] ; then
						continue
					fi
					#echo ""
				...
				#	file=`echo "$file" | tr '.' '_'`.dat
				#	file=$input/$file
				#	val=`grep "Time to setup queu" $file | cut -d',' -f17- | cut -d' ' -f5`
				#	#echo ""
				#	sum=`echo $sum+$val | bc`
				#	count=$(($count+1))
				#done
